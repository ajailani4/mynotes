...
class AddNotes : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        notes = intent.getSerializableExtra("Notes") as? Notes

        //When Save button is clicked
        if(notes == null)
        {
            save_btn.setOnClickListener {
                if(input_title.text.toString().isNotEmpty() && input_description.text.toString().isNotEmpty())
                {
                    notesViewModel.insertNotes(Notes(title = input_title.text.toString(), desc = input_description.text.toString()))

                    Toast.makeText(applicationContext, "Data is successfully added", Toast.LENGTH_SHORT).show()
                } else
                {
                    Toast.makeText(applicationContext, "Fill the form completely", Toast.LENGTH_SHORT).show()
                }
